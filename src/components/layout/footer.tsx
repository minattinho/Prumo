...
export function Footer() {
  return (
    <footer className="bg-azul-noite text-white mt-auto border-t border-azul-principal/20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          {navSections.map((section) => (
            <div key={section.title}>
              ...
              <div className="w-6 h-0.5 bg-laranja-obra mb-4" />
              <ul className="space-y-2 text-sm text-blue-200">
                {section.links.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="hover:text-white hover:underline underline-offset-4 transition-all duration-200 cursor-pointer"
                    >
                      {link.label}
                    </Link>
                  </li>
        ...
        <div className="mt-10 pt-6 border-t border-blue-800 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-blue-300">
          ...
          <p className="italic text-blue-400">Sua obra no prumo.</p>
          <div className="flex items-center gap-3">
            {socialLinks.map((s) => (
              <a
                ...
                rel="noopener noreferrer"
                aria-label={s.label}
                className="text-blue-400 hover:text-white transition-colors duration-200 cursor-pointer"
              >
                {s.icon}
              </a>
            ))}
